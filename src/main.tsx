  ...
  return (
    <main>
      <section className="hero grid-bg">
        <div className="eyebrow"><Sparkles size={15} /> Built for Privacy on Avalanche</div>
        ...
        <section className="form-card">
          {state === "success" ? <Success handle={handle} amount={amount} tx={lastTx} reset={() => setState("idle")} /> : (
            <>
              ...
              <div className="quick-amounts">{["10", "25", "50", "100"].map((value) => <button key={value} onClick={() => setAmount(value)} className={amount === value ? "active" : ""}>${value}</button>)}</div>
...
function Dashboard({ connection, live }: { connection: ConnectState; live?: LiveActions }) {
  ...
  return (
    <main className="page">
      <div className="dashboard-head"><div><span className="kicker">CREATOR STUDIO</span><h1>Welcome back, <em>@alice_streams</em></h1><p>Here is your confidential earnings overview.</p></div><div className="head-actions"><button className="secondary" onClick={() => navTo("/settings")}>Tax mode</button><button className="primary small" onClick={() => setRegistered(false)}>New profile</button></div></div>
